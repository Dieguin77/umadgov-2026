    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Pedido ${order.numeroPedido}`} size="lg">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div key={item.label} className="bg-gray-50 rounded-xl p-3">
              ...
              <p className="font-semibold text-lavanda-800">{item.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="border border-lavanda-100 rounded-xl p-4">
          <p className="text-xs text-lavanda-400 uppercase tracking-wide mb-3">Comprovante</p>
          {order.comprovante ? (
            <div className="flex items-center justify-between">
              <p className="text-lavanda-700 text-sm font-medium truncate flex-1">{order.comprovante}</p>
              <div className="flex gap-2 ml-3">
                <Button variant="outline" size="sm" icon={Eye}>Ver</Button>
                <Button variant="ghost" size="sm" icon={Download}>Baixar</Button>
              </div>
            </div>
          ) : (
            <p className="text-lavanda-400 text-sm italic">Comprovante não enviado</p>
          )}
        </div>
